/**
 * 紧急关键词检测器
 * 用于检测用户输入中的紧急求助信号，并在 AI 回复前/后给出安全提示
 */
 ...
 * @returns {Object|null} - 检测结果 { type, keywords, advice } 或 null
 */
export function detectEmergency(text) {
  ...
  for (const [type, config] of Object.entries(EMERGENCY_PATTERNS)) {
    const matched = config.keywords.filter((keyword) =>
...
/**
 * 生成紧急提示 HTML
 * @param {Object} emergency - detectEmergency 的返回值
 * @returns {string} - 提示 HTML
 */
export function generateEmergencyAlert(emergency) {
  if (!emergency) return ''
  ...
  return `
    <div class="elderly-emergency-alert">
      <div class="elderly-emergency-alert-title">
        ⚠️ 紧急安全提醒：${typeNames[emergency.type] || '紧急情况'}
      </div>
      <div>${emergency.advice}</div>
      <div style="margin-top: 8px; font-size: 14px; color: #999;">
        如情况紧急，请立即点击右下角"紧急求助"按钮拨打120。
      </div>
    </div>
  `
}
