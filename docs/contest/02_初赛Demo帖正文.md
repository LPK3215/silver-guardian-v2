...
* **主要功能：**
  ...
  **① 系统首页**
  系统首页采用琥珀主题设计，展示"银发守护 Silver Guardian — 智慧养老 AI 助手"品牌形象与产品定位，用户点击即可进入对话。

  ![系统首页 — 银发守护品牌 Landing Page，琥珀主题](product_06_skill_result.png)

  **② AI 对话 + 养老知识库 RAG**
  对话界面支持输入和发送消息，内置高频示例问题（"高龄津贴怎么申请""高血压患者饮食注意事项"等），点击可快速提问。系统内置 4 个养老场景智能体（银发健康顾问、养老政策助手、护理知识助手、生活陪伴，见下方智能体管理截图），基于 DeepSeek 大模型进行自然语言对话。知识库收录了 4 份养老领域文档（老年人权益保障法、慢性病管理指南、护理操作规范、防诈骗安全指南），通过 Milvus 向量检索实现语义搜索，AI 回答附带来源引用，老人可以追问"这句话出自哪份文件"。

  ![AI 对话界面 — 空态 + 示例问题 Chips](product_04_sos_emergency.png)

  ![智能体管理 — 4 个养老场景智能体（银发健康顾问、养老政策助手、护理知识助手、生活陪伴）](product_02_chat_followup.png)

  ![智能体管理 — 模型供应商配置（DeepSeek、SiliconFlow 等）](product_01_ai_chat_rag.png)
  ...
  **④ 紧急安全机制 — 急症检测 + SOS 求助**
  ...
  - **SOS 浮动按钮** — 对话界面右下角常驻红色 SOS 按钮（参见上方对话界面截图），点击后弹出紧急求助面板，一键拨打 120、110 或预设的紧急联系人电话；
  - **心理危机识别** — 当检测到"不想活了、想死"等表述时，自动提示 24 小时心理援助热线（400-161-9995）。

  **⑤ 知识库、工作区与养老专属 Skills**
  - **知识库管理** — 支持上传、浏览养老领域文档，通过 Milvus 向量检索构建可溯源的语义知识库；
  - **工作区文件管理** — 提供文件列表浏览功能，方便管理养老相关文档；
  - **工作台仪表盘** — 展示系统调用统计、活跃度曲线、知识库使用情况等数据总览；
  - **养老专属 Skills** — 提供老年人综合评估（ADL 日常生活能力、MMSE 认知功能、跌倒风险、营养状况、情绪状态）和个性化护理计划生成（日常安排、饮食方案、运动方案、用药管理）等技能。

  ![知识库管理 — 银发守护知识库文件列表](product_05_emergency_alert.png)

  ![工作区 — 文件列表浏览](product_07_skill_overview.png)

  ![工作台 — 数据仪表盘总览](product_03_elderly_voice_tts.png)
...
由于本项目为 AI 对话系统，需要后端服务和大模型 API 支持，无法直接部署为纯静态页面。因此通过 GitHub Pages 搭建了静态预览版本，展示项目核心界面和交互设计，供评审快速了解产品形态。

**GitHub Pages 预览地址：** [https://lpk3215.github.io/silver-guardian-v2/demo/](https://lpk3215.github.io/silver-guardian-v2/demo/)

> 注：该页面为静态演示版本，支持模拟对话交互（根据提问主题返回对应预设回复），知识库数据为预设示例，不涉及真实 AI 调用。进入页面后会显示提示弹窗，确认后即可体验产品界面并发送消息进行模拟对话。如需体验完整功能，请在本地通过 Docker 部署运行。
